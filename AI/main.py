...
def resize_image(image_path, size):
    """
    Resize an image to the specified size and return the resized image and its original size.
    """
...
def restore_image_size(image_tensor, original_size):
    """
    Restore the image tensor to its original size.
    """
    image_pil = Image.fromarray((image_tensor.permute(1, 2, 0).cpu().numpy() * 255).astype('uint8'))
    restored_image = image_pil.resize(original_size, Image.LANCZOS)
...
def main(model_args, args):
    ...
    for exp in tqdm(experiments):
        ...
        resized_face, original_size_face = resize_image(face_path, (1024, 1024))
        resized_shape, _ = resize_image(shape_path, (1024, 1024))
        resized_color, _ = resize_image(color_path, (1024, 1024))
        ...
        # Perform hair swapping
        final_image = hair_fast.swap(resized_face, resized_shape, resized_color, benchmark=args.benchmark, exp_name=exp_name)

        # Restore the original size
        restored_image = restore_image_size(final_image, original_size_face)
